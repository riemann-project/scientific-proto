  ...
  def link_to_useful reference
    if user_signed_in?
      if Useful.exists?(reference_id: reference.id, user_id: current_user.id)
        ...
        link_to "Unuseful", problem_reference_useful_path(answer.problem, reference, u), method: :delete, class: "btn"
      else
        link_to "Useful", problem_reference_usefuls_path(answer.problem, reference), method: :post, class: "btn"
      end
    else
